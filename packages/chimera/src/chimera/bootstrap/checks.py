"""Read-only drift checks for `chimera bootstrap --check` / `chimera doctor`.
...
anything: "what would `chimera bootstrap` do right now?"
"""
...
from chimera.bootstrap.operations import OpResult, _claude_mcp_list
...
def check_claude_hooks() -> OpResult:
    """Are chimera's hooks present in settings.json and using the
    current `python -m chimera.hooks.<name>` command form?

    A "yes" requires all three hook events (PostToolUse, SessionStart,
    UserPromptSubmit) to have a chimera-marked entry whose command
    starts with `<some-python> -m chimera.hooks.`. Legacy command
    forms (filesystem paths to scripts/hooks/*.py) report `would-update`
    so the user re-runs install-hooks.
    """
    try:
        from chimera.cli.install_hooks import _CHIMERA_MARKER, SETTINGS_PATH
    ...
    if not settings_path.is_file():
        return OpResult(
            ...
            status="created",
            detail="would create settings.json with chimera hooks",
        )
    ...
    for event in required:
        ...
        if not isinstance(entries, list):
            ...
            continue
        chimera_cmds: list[str] = []
        for matcher in entries:
            if not isinstance(matcher, dict):
                continue
            for h in matcher.get("hooks") or []:
                if isinstance(h, dict) and h.get(_CHIMERA_MARKER):
                    cmd = h.get("command") or ""
                    chimera_cmds.append(cmd)
        if not chimera_cmds:
            missing.append(event)
            continue
        # Look for at least one command using the current form.
        if not any("-m chimera.hooks." in c for c in chimera_cmds):
            legacy.append(event)
    ...
    if missing:
        return OpResult(
            ...
            target=str(settings_path),
            status="created",
            detail=f"would add chimera hooks for: {', '.join(missing)}",
        )
    if legacy:
        return OpResult(
            op="claude-hooks",
    ...
    return OpResult(
        ...
        target=str(settings_path),
        status="unchanged",
        detail="all 3 events use the current `python -m chimera.hooks.` form",
    )
...
def check_supervisor() -> OpResult:
    ...
    and matches what chimera would write today.
    """
    import sys

    try:
        if sys.platform == "linux":
            from chimera.monitor.cli import _systemd_unit_content, _systemd_unit_path

            path = _systemd_unit_path()
            label = "systemd user unit"
        elif sys.platform == "darwin":
            from chimera.monitor.cli import _launchd_plist_content, _launchd_plist_path
...
def check_spa(chimera_root: Path) -> OpResult:
