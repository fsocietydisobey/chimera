"""`chimera heal` — self-reflect, detect drift, apply fixes.

The "is anything broken? then fix it" command. Closes the loop between
`chimera doctor` (read-only diagnostic) and `chimera bootstrap`
(apply changes from a profile): heal runs the checks AND the fixes
in one shot, scoped to chimera's own setup health.

Heal scope:
  - Profile drift (symlinks missing/wrong, MCP servers unregistered,
    hooks pointing at legacy paths, etc.) → `chimera bootstrap` to apply
  - Daemon down → `chimera monitor start`
...
Invocation: `chimera heal [--profile <path>] [--aggressive] [--dry-run]`
...
def add_subparser(subparsers: argparse._SubParsersAction) -> None:
    p = subparsers.add_parser(
        "heal",
        help="Auto-detect chimera setup drift and apply fixes (doctor + bootstrap in one).",
        description=(
            "Runs the same checks as `chimera doctor`, then applies the "
            "fixes that doctor would surface as drift. Idempotent — safe "
            "on a healthy machine (no-op). Scoped to chimera's own setup; "
    ...
    p.add_argument(
        "--profile",
        default=None,
        help="Path or http(s) URL to a chimera profile YAML. Same resolution as bootstrap.",
    )
    p.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the planned fixes without applying anything (alias for `chimera doctor`).",
    )
    p.add_argument(
        "--aggressive",
        action="store_true",
        help=(
            "Also apply fixes with side effects beyond chimera state: "
...
def _run_heal(args: argparse.Namespace) -> int:
    """Three-phase heal:
      ...
      Phase 2: apply chimera-setup fixes (bootstrap + hooks reload).
    ...
    try:
        ...
        from chimera.bootstrap.runner import check_bootstrap, run_bootstrap
    except ImportError as e:
        print(f"❌ chimera.bootstrap import failed: {e}", file=sys.stderr)
        return 2
    ...
    if daemon_down:
        print("  • chimera-monitor daemon down")
    ...
    if drift_count or failed_count:
        print(f"  → running `chimera bootstrap`{' --force' if args.force else ''}…")
    ...
    if daemon_down:
        if args.aggressive:
            print("  → starting chimera-monitor daemon…")
            if _start_daemon():
                print("    ✓ daemon started")
                fixed += 1
            else:
                print("    ⚠ daemon start failed — check `chimera monitor status`")
        else:
    ...
    if not args.aggressive and daemon_down:
        print(
            "   Daemon is still down. Run `chimera heal --aggressive` "
            "or `chimera monitor start` to recover."
        )
    return 0
...
def _start_daemon() -> bool:
    ...
    try:
        from chimera.monitor.cli import _cmd_start
    except ImportError:
        return False
