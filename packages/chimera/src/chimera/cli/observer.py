"""`chimera observer` subcommand — query observer/heartbeat data from CLI.
...
def _get(path: str, base: str = _DEFAULT_BASE) -> dict:
    ...
    try:
        ...
        print(f"chimera-monitor unreachable at {base} ({e}). Start it with `chimera monitor`.", file=sys.stderr)
        sys.exit(2)
