...
Records every chimera MCP tool call to JSONL: timestamp, tool name, args
(truncated), elapsed, success, output size, error.

Answers questions like:
  - Which chimera tools is this agent actually using?
...
Storage: ~/.local/state/chimera/mcp-calls.jsonl. One line per call.
Sync writes — chimera-monitor daemon process serializes via asyncio.Lock.
...
def summarize(window_minutes: int = 60 * 24) -> dict[str, Any]:
    """Aggregate stats over the window. The 'is chimera being used effectively?'
