...
from chimera_types import RoutingDecision, TaskClassification

from chimera.config import is_local_only_mode, load_routing_table
...
def route(
    ...
    *,
    ...
    # Privacy gate — when CHIMERA_LOCAL_ONLY=1, only local runners are eligible
    if privacy_mode and recommended_runner not in local_only:
        ...
        for candidate in local_only:
            if RUNNERS.get(candidate) and RUNNERS[candidate].is_available():
                return RoutingDecision(
                    ...
                    fallback_reason=(
                        f"CHIMERA_LOCAL_ONLY=1; overrode "
        ...
        return RoutingDecision(
            ...
            refused=True,
            refusal_reason=(
                "CHIMERA_LOCAL_ONLY=1 but no local runner is available. "
    ...
    return RoutingDecision(
        ...
        refusal_reason=(
            ...
            "Run `chimera doctor` to diagnose."
        ),
    )
