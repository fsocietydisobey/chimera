"""CLI runner registry — the only place chimera talks to LLMs.

Pure subprocess substrate. No API SDK calls anywhere else in chimera.
...
def get_runner(name: str) -> CLIRunner:
    ...
    if runner is None:
        raise KeyError(
            ...
            "Add new runners by registering in chimera/dispatch/runners/__init__.py."
        )
    return runner
...
def available_runners() -> dict[str, bool]:
    ...
    Cheap — `is_available()` is just a `which` lookup. Used by `chimera doctor`
