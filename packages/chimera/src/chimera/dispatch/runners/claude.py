...
Wraps the `claude` binary (Claude Code) with chimera's session-resume,
extended-thinking, and permission-mode conventions. Migrated from
chimera-legacy `cli/runners.py`; cleaned up to drop the global session
...
DEFAULT_CMD = os.environ.get("CHIMERA_CLAUDE_CMD", "claude")
DEFAULT_MODEL = os.environ.get("CHIMERA_CLAUDE_MODEL", "claude-opus-4-7")
        ...
        **_: object,
        ...
        if not self.is_available():
            raise RuntimeError(
                ...
                "Set CHIMERA_CLAUDE_CMD or install Claude Code."
            )

        model_id = model or self.default_model

        # --strict-mcp-config + empty --mcp-config prevents the spawned
        # claude from loading chimera's own MCP entry — which would spawn
        # a child chimera that grabs the pidlock and kills its parent.
