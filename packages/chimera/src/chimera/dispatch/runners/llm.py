...
For chimera, this runner is the "long tail" runner. Devs who want to
experiment with a non-mainstream model (Mistral, DeepSeek, a specific
fine-tune on OpenRouter) install the relevant `llm` plugin and chimera
...
DEFAULT_CMD = os.environ.get("CHIMERA_LLM_CMD", "llm")
DEFAULT_MODEL = os.environ.get("CHIMERA_LLM_MODEL", "")  # No default — caller must specify
        ...
        **_: object,
        ...
        if not self.is_available():
            raise RuntimeError(
                ...
                "Set CHIMERA_LLM_CMD or install: pip install llm"
            )
        ...
        if not model_id:
            raise ValueError(
                ...
                "CHIMERA_LLM_MODEL or pass model= per call."
            )
