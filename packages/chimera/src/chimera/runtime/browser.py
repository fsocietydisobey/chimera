...
This isn't strictly required for `chimera dev` to work — the dev server
runs fine without browser orchestration. But devs who use chimera +
...
def build_launch_cmd(
    *,
    ...
    if user_data_dir is None:
        # Per-chimera-dev-session profile so we don't trample the user's main browser
        user_data_dir = tempfile.mkdtemp(prefix="chimera-dev-chrome-")
...
def installation_hint() -> str:
    """Helpful message when chimera dev runs and Chrome isn't found."""
