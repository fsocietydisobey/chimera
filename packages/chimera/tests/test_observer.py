"""Tests for chimera_observer — the venv-injected LangGraph instrumentation.

Covers:
  - tag_run() ContextVar set/reset
  - _enqueue() stamps correlation_id from the ContextVar
  - _should_skip_external() loop guard (skip chimera daemon host + loopback)
  - LangSmith bypass shim (no-op when CHIMERA_DISABLE_LANGSMITH=true)
...
# packages/chimera/src/chimera/attach/observer_template/chimera_observer/
_OBSERVER_PATH = (
    Path(__file__).resolve().parent.parent / "src" / "chimera" / "attach"
    / "observer_template"
)
...
def clean_observer(monkeypatch: pytest.MonkeyPatch):
    """Reload chimera_observer with a fresh module state per test.
    ...
    if "chimera_observer" in sys.modules:
        del sys.modules["chimera_observer"]
    import chimera_observer
    importlib.reload(chimera_observer)

    # Reset module-level singletons so tests are independent
    chimera_observer._attached = False
    chimera_observer._q = None
    chimera_observer._drain_thread = None
    yield chimera_observer
...
def test_should_skip_external_chimera_daemon_host(clean_observer):
    """Skip the chimera daemon's own host even if non-loopback (defensive)."""
    obs = clean_observer
    obs._endpoint = "http://my-chimera-host.local:8740"

    assert obs._should_skip_external("my-chimera-host.local") is True
...
def test_langsmith_bypass_no_op_when_disabled(clean_observer, monkeypatch):
    """When CHIMERA_DISABLE_LANGSMITH is unset, _patch_langsmith_bypass is a no-op."""
    obs = clean_observer
    monkeypatch.delenv("CHIMERA_DISABLE_LANGSMITH", raising=False)
...
def test_langsmith_bypass_skips_when_module_missing(clean_observer, monkeypatch):
    ...
    obs = clean_observer
    monkeypatch.setenv("CHIMERA_DISABLE_LANGSMITH", "true")
