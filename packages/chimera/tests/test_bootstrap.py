...
from chimera.bootstrap.runner import run_bootstrap, run_sync
...
def test_load_profile_defaults_when_nothing_else_set(monkeypatch):
    ...
    monkeypatch.delenv("CHIMERA_PROFILE", raising=False)
    ...
    assert profile.name == "chimera-default"
...
def test_register_mcp_skips_when_claude_cli_missing():
    ...
    """
    with patch("chimera.bootstrap.operations.shutil.which", return_value=None):
...
def test_run_sync_dotfiles_pull_idempotent(fake_dotfiles, tmp_path):
    """`chimera sync` against an already-current repo reports unchanged.
