...
- **Auto-accept invites from trusted peers**: today every invite requires an explicit `chat_accept` (handshake by design — "phone call where both parties answer"). For master/agent flows where the user fully trusts certain peers (e.g., agents they spawned themselves), the handshake is friction. Add a per-session config: `chat_auto_accept_from(session_id, allowlist=["khimaira-21", "test-master"])` or a global toggle for "accept all invites in this session." Joseph asked about this when watching agents accept multiple invites across smoke rounds.
