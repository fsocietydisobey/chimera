"""RuntimeStatus — `chimera dev`'s view of the running stack.

Polled by the dashboard's runtime panel ('is the dev server up? is Chrome
attached? is the DB connected?'). Also returned by `chimera doctor`.
"""
...
class ComponentHealth(BaseModel):
    """One subsystem chimera dev manages."""
...
class RuntimeStatus(BaseModel):
    """Composite status of the whole `chimera dev` stack."""
