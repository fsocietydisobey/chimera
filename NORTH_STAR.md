...
> **Chimera is the orchestration layer that lives below any AI tool.** One
> MCP config line connects the user's editor to a single chimera server
...
1. **Editor-agnostic via MCP.** We build infrastructure that any
   ...
   configs live in `contrib/` as examples, not in chimera core.

2. **No manual scripts as the primary install path.** The user adds one
   line to their MCP config. Chimera self-configures on first connect.
   Shell commands remain for non-MCP scenarios; they are not the pitch.

3. **Invisible infrastructure.** Chimera does NOT replace editors, ship
...
4. **Consolidate, don't fragment.** seance, specter, scarlet, and any
   future chimera-family capability lives in one workspace, one process,
...
5. **Enhance the existing agent ecosystem, do not compete with it.**
   ...
   of chimera, not competitors. Chimera makes them smarter; it does not
   try to be them.
...
- **Routing engine**: classifier, pool router, registry
  (`~/.chimera/models.yaml`), capability-aware model selection across
  claude/codex/gemini/ollama/llm runners.
- **MCP surface**: `mcp__chimera__auto`, `delegate`, `chain`, plus
...
- **Usage tracking**: every dispatch recorded with mode
  ...
  estimate. `chimera usage savings` computes Opus-direct counterfactual.
- **Observer**: multi-session state, handoffs, decisions, notices,
  LangGraph trace waterfall, daemon supervisor.
- **Bootstrap framework**: profile-driven (`chimera-profile.yaml`),
  cross-machine portable. `chimera bootstrap`, `chimera doctor`,
  `chimera heal` cover install, drift detection, self-healing.
...
- **Workspace consolidation (code-level)**: `packages/seance`,
  `packages/specter`, `packages/scarlet`, `packages/chimera` all share
...
Today: 4 separate MCP servers (chimera, seance, specter, scarlet).
After: one chimera MCP exposes everyone's tools.

- Collapse duplicate copies at `~/dev/{seance,specter,scarlet}` (keep
  the `packages/` versions as canonical).
- Import seance/specter/scarlet tools into chimera's MCP server,
  re-expose under one connection.
- Deprecate the standalone `seance serve` / `specter serve` /
  `scarlet serve` commands.

**Done when**: `claude mcp list` shows one chimera entry with all tools.

### Phase 1.0 — MCP-first self-configuration  (3-5 days)

Today: user runs `chimera bootstrap`. After: user adds one MCP line,
chimera configures itself.
...
**Done when**: A new user adds `"chimera": { "command": "uvx", "args":
["chimera-orchestrator", "mcp"] }` to their MCP config, restarts their
...
tool without reading chimera's source.

### Phase 1.2 — Subagent library  (3-4 days)

`~/.claude/agents/chimera-*.md` curated set, each pinned to the right
model. Real thinking-token interception inside Claude Code.

- chimera-factual (haiku), chimera-grep (haiku), chimera-code-fast
  (haiku), chimera-research (gemini-flash/sonnet), chimera-code-deep
  (sonnet), chimera-architect (opus), chimera-debug (sonnet) →
  chimera-deep-debug (opus) escalation pair.
...
`contrib/` examples, not chimera core. Demonstrates that the protocol
...
- 2.5 `docs/INTEGRATING.md` — the canonical "integrate chimera into
  your AI tool" guide
...
- 3.1 PyPI package (decide name; bare `chimera` is taken)
- 3.2 README rewrite — lead with savings, frame as orchestration layer
- 3.3 Community profile (`chimera-profile.yaml` pointing at public repos)
- 3.4 Demo assets (GIFs + 3-minute walkthrough)

**Done when**: `uvx chimera-orchestrator mcp` works on a fresh laptop.
...
- 4.1 Claude Agent SDK investigation (subscription auth + per-call
  model swap path for chimera dispatches)
...
- **A separate MCP server per capability.** seance/specter/scarlet are
  capabilities of chimera, not peer servers. One MCP, many tools.
- **Re-inventing classification/routing logic in each editor adapter.**
  Routing lives in chimera core. Adapters call it.
...
- **Commit + push the auto-mode work shipped this session** — pool
  router, registry, `mcp__chimera__auto`, mode field on UsageRecord,
  `chimera usage savings` command, 19 new tests. Currently uncommitted.
- **README update for new features** — `mcp__chimera__auto`,
  `chimera usage savings`, the registry at `~/.chimera/models.yaml`,
...
- **`_COUNTERFACTUAL_MODEL` in `usage.py` is hardcoded** to
  ...
  instead" baselines.
- **Auto-route audit log lives only in `chimera.log`** — grep-only,
  no structured viewer. Phase 4.4 (dashboard polish) addresses this;
  in the meantime, the `chimera usage list --mode auto` command
  partially fills the gap.

### Quality / robustness gaps

- **No rate-limit / quota-exhaustion handling in dispatch path.** If
  a runner returns 429, chimera surfaces the error to the caller but
...
- **No circuit breakers when a runner repeatedly fails.** Same shape
  as above — if `claude` is broken, chimera should stop trying it for
...
- **`mcp__chimera__auto` and `delegate` end-to-end tests.** The
...
- **Audit-log assertion tests.** We log classifier_confidence,
  ...
  land in `chimera.log` in the expected shape.
...
- **PyPI package name.** Bare `chimera` is taken on PyPI. Candidates:
  `chimera-orchestrator`, `chimera-router`, `chimera-ai`,
  `chimera-mcp`. Pick before Phase 3.1.
...
- **Single repo vs split.** Should adapter configs ship in
  `chimera/contrib/` or as a separate `chimera-adapters` repo?
...
- **avante.nvim provider API.** Their provider abstraction may or
  may not match what chimera exposes. Need to read avante source
...
- **Claude Code transcript format for Phase 4.2.** JSONL shape
  ...
  savings command can include non-chimera-routed dispatches.
...
- **No per-project model budget enforcement.** `--budget` flag exists
  on `chimera task` but not on `mcp__chimera__auto`. Should surface.
...
- **No multi-turn conversation through `mcp__chimera__auto`.** Each
  call is one-shot. Multi-turn would need session state on chimera's
...
- **No automatic model registry refresh.** When Anthropic / Google /
  ...
  `~/.chimera/models.yaml`. A `chimera models sync` command pulling
...
- **Building a chimera TUI / IDE.** Six-month project, wrong fight,
  loses the "lives below the editor" frame.
- **Per-editor plugins maintained in chimera core.** Adapter configs
...
- Memory + persistent context live in
  `~/.claude/projects/-home--3ntropy-dev-chimera/memory/`. See
...
- Session coordination via `mcp__chimera__session_*`. Cross-session
